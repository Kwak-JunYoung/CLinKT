...
class SAKT(Module):
    def __init__(
        ...
        self.interaction_emb = Embedding(
            num_skills * 2, embedding_size, padding_idx=0)

        self.exercise_emb = Embedding(
            num_skills, embedding_size, padding_idx=0)

        # self.P = Parameter(torch.Tensor(self.seq_len, self.embedding_size))
        self.position_emb = Embedding(
            seq_len + 1, embedding_size, padding_idx=0)

        self.blocks = get_clones(
            Blocks(device, embedding_size, num_attn_heads, dropout), self.num_blocks)
    ...
    def base_emb(self, q, r, qry, pos):
        ...
        interaction_input_matrix = xemb + posemb
        return qshftemb, interaction_input_matrix
...
class Blocks(Module):
    def __init__(self, device, embedding_size, num_attn_heads, dropout) -> None:
        ...
        self.attn = MultiheadAttention(
            embedding_size, num_attn_heads, dropout=dropout)
